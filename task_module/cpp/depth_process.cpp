...
namespace PointCloud {
    ...
    void thread_PointCloudProcess() {
        ...
        uint64_t producedDepthFrames = 0;
        uint64_t emptyDepthFrames = 0;
        uint64_t computeSamples = 0;
        long long computeTotalMs = 0;
        long long computeMaxMs = 0;
        bool hasLastReadReturn = false;
        auto lastReadReturn = std::chrono::steady_clock::now();
        uint64_t readIntervalSamples = 0;
        long long readIntervalTotalMs = 0;
        long long readIntervalMaxMs = 0;
        ...
        while (1)
        {
            ...
            if (g_sysConfig.workMode == UartComm::WorkMode::STANDARD) {
                ...
                auto readReturnTime = std::chrono::steady_clock::now();

                if (depthImg.isEmptyFrame()) {
                    ++emptyDepthFrames;
                    Logger::instance().debug("Thread PointCloudProcess - Failed to read point cloud from PCIe");
                } else {
                    if (hasLastReadReturn) {
                        long long readIntervalMs = std::chrono::duration_cast<std::chrono::milliseconds>(readReturnTime - lastReadReturn).count();
                        readIntervalTotalMs += readIntervalMs;
                        if (readIntervalMs > readIntervalMaxMs) {
                            readIntervalMaxMs = readIntervalMs;
                        }
                        ++readIntervalSamples;
                    }
                    lastReadReturn = readReturnTime;
                    hasLastReadReturn = true;
                ...
                computeTotalMs += duration_ms;
                if (duration_ms > computeMaxMs) {
                    computeMaxMs = duration_ms;
                }
                ++computeSamples;
                ...
                if (elapsedMs >= 1000) {
                    ...
                    const double producedFps = static_cast<double>(producedDepthFrames) / sec;
                    const double avgComputeMs = (computeSamples > 0) ? (static_cast<double>(computeTotalMs) / computeSamples) : 0.0;
                    const double avgReadIntervalMs = (readIntervalSamples > 0) ? (static_cast<double>(readIntervalTotalMs) / readIntervalSamples) : 0.0;

                    Logger::instance().debug((
                        "PointCloudStats "
                        "produced=" + std::to_string(producedDepthFrames) +
                        ", produced_fps=" + std::to_string(producedFps) +
                        ", empty=" + std::to_string(emptyDepthFrames) +
                        ", compute_avg_ms=" + std::to_string(avgComputeMs) +
                        ", compute_max_ms=" + std::to_string(computeMaxMs) +
                        ", read_interval_avg_ms=" + std::to_string(avgReadIntervalMs) +
                        ", read_interval_max_ms=" + std::to_string(readIntervalMaxMs)
                    ).c_str());

                    reportStart = now;
                    producedDepthFrames = 0;
                    emptyDepthFrames = 0;
                    computeSamples = 0;
                    computeTotalMs = 0;
                    computeMaxMs = 0;
                    readIntervalSamples = 0;
                    readIntervalTotalMs = 0;
                    readIntervalMaxMs = 0;
                }
            }
        }
