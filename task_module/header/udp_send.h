...
namespace UdpComm {
    ...
    struct SendDataStats {
        size_t checksumCalculated = 0;
        size_t frameSendOk = 0;
        size_t frameSendFail = 0;
        size_t retryHits = 0;
        size_t eagainHits = 0;
    };
    ...
    class UdpSender {
        public:
            ...
            // 发送大数据，自动分片组帧
            bool sendData(const uint8_t* data, size_t length, uint32_t transfer_id, uint8_t task_type, int16_t pitch = 0, int16_t yaw = 0, SendDataStats* outStats = nullptr);
        ...
        private:
            ...
            // 组帧并发送单个分片
            bool sendFrame(const UdpFrame& frame, size_t* outRetryHits = nullptr, size_t* outEagainHits = nullptr);
